...
export default function Translate({ className }: { className?: string }) {
  const [query, setQuery] = useState("The UK drinks about 100–160 million cups of tea every day, and 98% of tea drinkers add milk to their tea.");
  ...
  return (
    <div className={cn("bg-zinc-100 dark:bg-zinc-900 rounded-xl p-6 flex flex-col gap-4", className)}>
      <div className="flex flex-col gap-2">
        ...
        <p className="text-zinc-500 dark:text-zinc-400 text-sm">Translate text to different languages using AI.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <Button type="submit" disabled={loading || !query.trim() || !targetLanguage} className="w-full">
      ...
      {results && !loading && (
        <div className="p-4 bg-white dark:bg-zinc-800 rounded-lg border">
          <p className="text-zinc-900 dark:text-zinc-100 font-medium">{results.translated_text}</p>
        </div>
      )}
    </div>
  );
